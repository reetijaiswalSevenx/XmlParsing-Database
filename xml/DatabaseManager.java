        ...
        try {
            ...
            for (Employee em : ListOfEmployees) {
                if (em.getEmployeeId() == 0) {
                    statement.execute("INSERT INTO `employeemanagement`.`employee` (`firstName`, `lastName`, `email`, `baseSalary`) VALUES ('" + em.getFirstName() + "', '" + em.getLastName() + "', '"+em.getEmail() +
                            "', '" + em.getBaseSalary() + "');");
                } else {
                    statement.executeUpdate("UPDATE `employeemanagement`.`employee` SET `firstName` = '" + em.getFirstName() + "', `lastName` = '" + em.getLastName() + "', `email` = '"+em.getEmail() +
                            "', `baseSalary` = '" + em.getBaseSalary() + "' WHERE (`employeeId` = '" + em.getEmployeeId() + "');");
                }
            }
